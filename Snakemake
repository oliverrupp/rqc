...
(PLANTS2, SAMPLES,)=glob_wildcards('{plants}/reads/{samples}_1.fq.gz')
...
def filter_combinator(combinator):
    def filtered_combinator(*args, **kwargs):
        ...
        for wc_comb in combinator(*args, **kwargs):
            filetocheck = "%s/reads/%s_1.fq.gz" % (wc_comb[0][1], wc_comb[1][1])
...
rule trim:
    input:
        expand('{plant}/results/trimmed/{sample}.json',
               filtered_product, plant=PLANTS, sample=SAMPLES)
...
rule quant_small:
    input:
        expand('{plant}/esults/salmon_{ref}/{sample}/quant.sf',
               filtered_product, plant=PLANTS, sample=SAMPLES, ref=['rrna', 'quantiles'])
    output:
