...
for(sample in samples) {
  ...
  if(file.exists(jfile)) {
    ...
    stats$reference <- "Gene"
    results <- rbind(results, stats)
    results[results$status == "Unmapped" & results$sample == sample,]$reads = 
            results[results$status == "Unmapped" & results$sample == sample,]$reads  - jdat$num_mapped
  }
}

results$status = factor(results$status, levels=rev(c("Assigned", "rRNA", "NoFeature", "Unmapped",
...
if(!dir.exists(bfcDir)) { dir.create(bfcDir) }
...
df = data.frame("G0"=colSums(gene_count_matrix == 0),
                "G1"=colSums(gene_count_matrix > 0 & gene_count_matrix <= 10),
                "G10"=colSums(gene_count_matrix > 10 & gene_count_matrix < 25),
...
for (sample_name in unique(sample_info$condition)) {
  ...
  if(sum(sample_info$condition == sample_name) > 1) {
  	...
  	dfm <- rbind(dfm, list(Replicate = sample_name, Sample = sample_name, variable="G1", value=sum(max_counts > 0 & max_counts <= 10)))
  	dfm <- rbind(dfm, list(Replicate = sample_name, Sample = sample_name, variable="G10", value=sum(max_counts > 10 & max_counts < 25)))
  	dfm <- rbind(dfm, list(Replicate = sample_name, Sample = sample_name, variable="G100", value=sum(max_counts >= 25)))
  }
}
...
dfm$variable = factor(dfm$variable, levels=c("No reads", "Low read count", "Medium read count", "Good read count"))
...
ComplexHeatmap::pheatmap(deg_matrix, cluster_rows = T, cluster_cols = F,
         treeheight_row=0, fontsize = 14, border = F,
         heatmap_legend_param=list(title="coverage (%)"), main="RNA degradation (gene body coverage)")
...
pheatmap::pheatmap(sample_cor, fontsize = 14, annotation_col=a, main="Sample Correlation Heatmap of VST counts")

PCA = pca(cor_data, metadata = colData(se))
biplot(PCA, x = "PC2", y = "PC1", colby="condition", title="PCA of VST counts")
...
if(length(low_q_samples) > 0 && length(low_q_samples) < ncol(cor_data)) {
  ...
  sample_dist_filtered <- as.matrix(dist(t(cor_data_filtered)))
   ...
   pheatmap::pheatmap(sample_cor_filtered, fontsize = 10,
           annotation_col=a, main="Filtered Sample Correlation Heatmap")

  PCA = pca(cor_data_filtered, metadata = colData(se)[! rownames(colData(se)) %in% low_q_samples,])
  biplot(PCA, x = "PC2", y = "PC1", colby="condition", title="PCA of VST counts (filtered samples)")
}
...
xsheet1 = pivot_wider(results[results$reference == "Gene",], id_cols="sample", names_from="status", values_from="reads")

xsheet1$trimmed = xsheet1$LowQuality+xsheet1$Nreads+xsheet1$TooShort+xsheet1$TooLong

xsheet1 = xsheet1 %>% dplyr::select(-one_of("LowQuality", "Nreads", "TooShort", "TooLong"))
...
write.table(xsheet1, paste0(outfile, ".report.tsv"), sep="\t", quote=F, col.names=TRUE, row.names=FALSE, append=FALSE)
write.xlsx2(xsheet1, paste0(outfile, ".report.xlsx"), sheetName="sequencing results", col.names=TRUE, row.names=FALSE, append=FALSE)

write.xlsx2(xsheet2, paste0(outfile, ".report.xlsx"), sheetName="reads per gene", col.names=TRUE, row.names=FALSE, append=TRUE)

write.xlsx2(xsheet3, paste0(outfile, ".report.xlsx"), sheetName="sample correlation", col.names=TRUE, row.names=TRUE, append=TRUE)

write.table(xsheet4, paste0(outfile, ".raw.counts.tsv"), sep="\t", quote=F, col.names=TRUE, row.names=TRUE, append=TRUE)

write.table(xsheet5, paste0(outfile, ".TMP.normalized.tsv"), sep="\t", quote=F, col.names=TRUE, row.names=TRUE, append=FALSE)
